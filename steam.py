...
class Steam():
    ...
    def applist_to_json(self, applist):
    ...
    async def save_applist_to_local(self, applist):
        ...
        content = self.applist_to_json(applist)
        if (content):
            async with aiofiles.open(APPLIST_LOCAL, 'w', encoding='utf8') as f:
                await f.write(content)


    async def get_applist_from_local(self, filename='AppList.json'):
        ...
        if (os.path.exists(APPLIST_LOCAL)):
            async with aiofiles.open(APPLIST_LOCAL, 'r', encoding='utf8') as f:
                ...
                return self.get_games_from_applist(applist)
        return {}
    ...
    def get_store_link(self, appid, typ):
    ...
    async def get_store_info_from_url(self, game, name, url):
    ...
    def get_document(self, page, name):
    ...
    async def get_store_info(self, game, name):
        page, description, url     = await self.get_page(game.store.link, name)
        if (not page):
            game.store.description = description
            return False

        document, description = self.get_document(page, name)
        ...
        # middle right column
        game.store.genres     = self.get_game_genres(document)
        game.store.details    = self.get_game_details(document)

        return True


    def get_standalone_info(self, game, name, document):
        # middle left column
        ...
        game.store.category = self.get_game_category(purchase_block, document)
        game.store.price    = self.get_game_price(purchase_block, name)
        ...
        game.store.cnt_review   = self.get_game_review(glance_ctn_block, name)
        game.store.release_date = self.get_game_release_date(glance_ctn_block, name)
        game.store.tags         = self.get_game_tags(glance_ctn_block)

        if (game.store.category in [Category.Game, Category.Video]):
            game.store.description  = self.get_game_description(glance_ctn_block)
        elif (game.store.category is Category.DLC):
            game.store.description  = self.get_dlc_description(document)
        else:
        ...
        game.store.audio,\
        game.store.subtitles    = self.get_game_languages(document)


    async def get_collection_info(self, game, name, document):
        ...
        for item in items:
            ...
            await self.get_store_info(itemgame, itemname)
        ...
        game.store.image            = self.get_collection_image(game_left_column)
    ...
    def get_dlc_description(self, document):
    ...
    def get_game_review(self, glance_ctn_block, name):
    ...
    def get_game_release_date(self, glance_ctn_block, name):
    ...
    def get_game_category(self, purchase_block, document):
    ...
    def get_game_price(self, purchase_block, name):
    ...
    def get_game_os(self, game_left_column):
    ...
    def get_game_genres(self, document):
    ...
    def get_game_details(self, document):
    ...
    def get_game_tags(self, glance_ctn_block):
    ...
    def get_game_languages(self, document):
        interface = list()
        audio     = list()
        subtitles = list()
    ...
    def get_titles(self, document, shortlink):
        titles = {}
...
if __name__ == '__main__':
    ...
    loop.set_debug(True)
    steam = Steam()
    storelink   = steam.get_store_link(appid, typ)
    try:
        page, _, _  = loop.run_until_complete(steam.get_page(storelink, name))
        document, _ = steam.get_document(page, name)
        titles      = steam.get_titles(document, '{}/{}'.format(typ, appid))
        print(titles)
    finally:
