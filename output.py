...
def output_to_html(games, file):
    ...
    writeline(
        f, '<script src="https://code.jquery.com/jquery-1.11.3.min.js"></script>')
    writeline(
        f, '<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>')
    ...
    writeline(f, 'format: "' + justifyFormat.replace(';',
                                                     '; text-indent:1em;', 1) + '",')
    writeline(f, 'unique: "true",')
    ...
    for gameName in sorted(games):
        ...
        if (game.link != ''):
            writeline(f, 'nameFormat: "<a href=\\"' + game.link + '\\"><b>' + justifyFormat +
        ...
        if (game.price == None):
            writeline(f, 'priceFormat: "' + justifyFormat.replace(
            ...
            if (game.price == 0):
                ...
                writeline(f, 'priceFormat: "' + justifyFormat.replace('{0}', '$' + str(
        ...
        if (game.avg_review.lower() in reviewMapping):
            ...
            writeline(f, 'review: "' + avg_review +
                      ' ' + game.avg_review + '",')
            writeline(f, 'reviewFormat: "' + justifyFormat.replace(
